#ifndef TW_TREIBER_STACK_H
#define TW_TREIBER_STACK_H

#include <threadweave/internal/Hazard.h>
#include <threadweave/internal/Node.h>
#include <threadweave/internal/NodeAllocator.h>
#include <threadweave/internal/utils.h>

#include <atomic>
#include <optional>
#include <type_traits>
#include <utility>

namespace ThreadWeave {

/**
 * An implementation of a lock-free (Treiber) stack data structure
 * @tparam T type of the object to store in the stack
 */
template <typename T>
  requires(std::is_nothrow_default_constructible_v<T> &&
           std::is_nothrow_move_constructible_v<T> &&
           std::is_nothrow_move_assignable_v<T>)
class alignas(Internal::kCacheLineSize) TreiberStack {
  // --- Data members
  using Node = Internal::StackNode<T>;
  using Allocator = Internal::NodeAllocator<Node>;
  alignas(Internal::kCacheLineSize) std::atomic<Node*> head_{nullptr};

 public:
  // --- Ctors, dtor, and assignment operators

  /**
   * Default construct a Treiber stack
   */
  TreiberStack() = default;

  /**
   * Free all memory associated with the stack
   */
  ~TreiberStack();

  // Prevent copy and move operations
  TreiberStack(const TreiberStack&) = delete;
  TreiberStack(TreiberStack&&) = delete;
  TreiberStack& operator=(const TreiberStack&) = delete;
  TreiberStack& operator=(TreiberStack&&) = delete;

  // --- Member functions

  /**
   * Push data to the top of the stack
   * @param data data to be inserted on top of the stack
   */
  void push(T data);

  /**
   * Pop data from the top of the stack
   * @return data popped from the stack (will be std::nullopt if the stack is
   * empty)
   */
  std::optional<T> pop();

  /**
   * Check if the stack is empty.
   * @return true if the stack is empty and false otherwise
   */
  bool empty() const noexcept;
};

template <typename T>
  requires(std::is_nothrow_default_constructible_v<T> &&
           std::is_nothrow_move_constructible_v<T> &&
           std::is_nothrow_move_assignable_v<T>)
TreiberStack<T>::~TreiberStack() {
  Node* head{head_.load(MemoryOrder::relaxed)};

  while (head) {
    Node* const curr{head};
    head = head->next;
    Allocator::deallocate(curr);
  }
}

template <typename T>
  requires(std::is_nothrow_default_constructible_v<T> &&
           std::is_nothrow_move_constructible_v<T> &&
           std::is_nothrow_move_assignable_v<T>)
void TreiberStack<T>::push(T data) {
  // Grab raw, recycled node memory from the block allocator (may very rarely
  // perform a heap allocation which is not lock-free)
  Node* const pushNode{Allocator::allocate()};
  TW_ASSERT(pushNode != nullptr, "Allocator returned null node in push()");

  // Move data into the node
  pushNode->data = std::move(data);

  // Push new node onto the stack
  pushNode->next = head_.load(MemoryOrder::relaxed);
  while (!head_.compare_exchange_weak(
      pushNode->next, pushNode, MemoryOrder::release, MemoryOrder::relaxed)) {}
}

template <typename T>
  requires(std::is_nothrow_default_constructible_v<T> &&
           std::is_nothrow_move_constructible_v<T> &&
           std::is_nothrow_move_assignable_v<T>)
std::optional<T> TreiberStack<T>::pop() {
  // Pointer to node on top of the stack
  Node* popNode{nullptr};

  {
    // Use an RAII guard for clearing hazard pointer when node is no longer in
    // use
    Internal::HazardGuard<Internal::HazardSlot::Stack0> headGuard{};

    do {
      // Acquire pointer pointing to head node with a hazard pointer indicating
      // current thread's use
      popNode = headGuard.acquirePointerWithHazard(head_);

      // Profiling demonstrated better performance with strong CAS
    } while (popNode && !head_.compare_exchange_strong(popNode, popNode->next,
                                                       MemoryOrder::acquire,
                                                       MemoryOrder::relaxed));
  }

  // Empty stack
  if (!popNode) {
    return std::nullopt;
  }

  // Data should be no throw move constructible and std::optional requires
  // std::is_nothrow_default_constructible_v<T> && no heap allocation so this
  // should be safe. Note that it is safe to dereference popNode at this point.
  // Only the current thread could've gotten out of the CAS loop with popNode
  // unlinked from the list. Thus, while other threads may have a pointer
  // pointing to same memory location, they are stuck in the CAS loop and only
  // this thread can delete it or add it to the retire list which happens after
  // this operation.
  std::optional<T> res{std::move(popNode->data)};

  // Hand the node back to the allocator. It automatically checks hazard
  // pointers and places it in either the local free list or local save list.
  Allocator::deallocate(popNode);
  return res;
}

template <typename T>
  requires(std::is_nothrow_default_constructible_v<T> &&
           std::is_nothrow_move_constructible_v<T> &&
           std::is_nothrow_move_assignable_v<T>)
bool TreiberStack<T>::empty() const noexcept {
  return head_.load(MemoryOrder::relaxed) == nullptr;
}

}  // namespace ThreadWeave

#endif

...
// of the thread pool. This shows TreiberStack use under MPMC semantics.
//
// Build:
//   g++ -std=c++23 -Iinclude -pthread 04_lock_free_queue.cpp -o lock_free_stack
// Run:
//   ./lock_free_stack
...
int main() {
  ThreadWeave::TreiberStack<int> stack{};
  ...
  // Multiple consumers drain the stack concurrently until producers are done
  std::vector<std::jthread> consumers{};

  for (int c{0}; c < kNumConsumers; ++c) {
    consumers.emplace_back([&] {
      while (producersRemaining.load(std::memory_order::acquire) > 0) {
        // Pop returns std::optional where std::nullopt indicates an empty stack
        if (stack.pop()) {
